...
const telemetryEvents: LogFields[] = [];

function assertSamlContinuity(t: Test, rows: LogFields[], relayState: string, subjectSource: string) {
  const stages = [
    'Polis SAML redirect issued',
    'Polis authorization code issued',
    'Polis token redemption succeeded',
    'Polis userinfo served',
  ].map((msg) => rows.find((row) => row.msg === msg)!);
  t.ok(stages.every(Boolean), 'the real controller flow logs the successful SAML actions');
...
tap.before(async () => {
  ...
  const controller = await (
    ...
    logger: {
      info: (msg, fields) => {
        telemetryEvents.push({ ...fields, msg, level: 'info' });
      },
      warn: (msg, fields) => {
        telemetryEvents.push({ ...fields, msg, level: 'warn' });
      },
      error: (msg, fields) => {
        telemetryEvents.push({ ...fields, msg, level: 'error' });
      },
    },
  });
