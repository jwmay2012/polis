  ...
  return async (url: string | URL, options: CustomFetchOptions): Promise<Response> => {
    return new Promise((resolve, reject) => {
      ...
      const req = request(requestOptions, (res) => {
        ...
        res.on('end', () => {
          telemetry.event('polis_upstream_response', {
            upstream_endpoint: endpoint,
            upstream_endpoint_role: endpointRole,
            upstream_http_method: requestOptions.method,
      ...
      req.on('error', (error) => {
        ...
        telemetry.attachErrorContext(error, facts);
        telemetry.event(
          'polis_upstream_request_failed',
          {
            ...facts,
            network_error_code: (error as NodeJS.ErrnoException).code,
            error_message: error.message,
          },
          'Polis upstream request failed',
          'error'
        );
