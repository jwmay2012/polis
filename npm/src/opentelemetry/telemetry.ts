...
export { attachErrorContext } from './errors';
...
// Initialize process-lifetime resources without capturing the first request.
// Exiting our ALS alone would still leave the SDK's active trace context.
...
export function bindSession(session: any, sessionId?: string) {
  safely(() => {
    ...
    enrich({
      ...
      upstream_redirect_uri:
        storage.getStore()?.fields.upstream_redirect_uri ??
